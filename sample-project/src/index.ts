...
server.tool(
  ...
  "Check if a documentation file contains all required sections for a complete API reference. " +
...
server.tool(
  ...
  "Extract API endpoints mentioned in a documentation file. Scans for HTTP method + path " +
...
server.tool(
  "grade_readability",
  "Calculate the readability score of a documentation file using the Flesch-Kincaid formula. " +
  ...
  async ({ filePath }) => {
    ...
    let category: string;
    if (roundedGrade <= 6) category = "Easy — suitable for general audience";
    ...
    else category = "Very difficult — strongly recommend simplification";
    ...
    if (avgWordsPerSentence > 25) {
      recommendations.push(
        `Average sentence length is ${avgWordsPerSentence} words — well above the recommended 20. Break long sentences into shorter ones.`
      );
    } else if (avgWordsPerSentence > 20) {
      recommendations.push(
        `Average sentence length is ${avgWordsPerSentence} words. Consider keeping most sentences under 20 words.`
      );
    }

    if (avgSyllablesPerWord > 1.8) {
      recommendations.push(
        `Average word complexity is high (${avgSyllablesPerWord} syllables/word). Use simpler vocabulary where possible.`
      );
    } else if (avgSyllablesPerWord > 1.5) {
      recommendations.push(
        `Word complexity is moderate (${avgSyllablesPerWord} syllables/word). Look for opportunities to simplify technical jargon.`
      );
    }

    if (roundedGrade > 14) {
      recommendations.push(
        "Grade level exceeds 14 — this is graduate-level difficulty. Strongly consider rewriting complex sections."
      );
    } else if (roundedGrade > 12) {
      recommendations.push(
        "Grade level exceeds 12. Consider breaking complex sentences and using plainer language for broader accessibility."
      );
    }

    if (sentences.some((s) => s.split(/\s+/).length > 40)) {
      recommendations.push(
        "Some sentences exceed 40 words. These should be split for clarity."
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "Readability is good! The document is well-suited for a technical audience."
      );
    }
